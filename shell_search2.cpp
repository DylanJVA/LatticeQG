...
tuple<vector<deque<int>>,int> get_shells(unordered_map<int, vector<int>> lattice, int starting_simplex) {
    ...
    // List of the shells, where each shell is a lists of simplices 
...
void get_return_prob(unordered_map<int, vector<int>> lattice, int origin, int num_walks, int max_steps) {
    srand(time(0));
    ...
    // We want to find the return probability for each number of steps
    double return_probs[max_steps] = {};
    
    for (int walk = 0; walk < num_walks; walk++) {
        int current_simplex = origin;
        for (int step = 0; step < max_steps; step++) {
            // Choose a random side to step to
            current_simplex = lattice[current_simplex][rand() % num_sides];
            if (current_simplex == origin)
                return_probs[step] += (double)1./num_walks;
        }
    }
    cout << "Number of steps,Return probability" << endl;
    for (int step = 0; step < max_steps; step++) cout << step+1 << "," << return_probs[step] << endl;
}
...
int main(int argc, char **argv) {
    ...
    // Print return probability vs number of steps in each walk
    get_return_prob(lattice, origin, stoi(argv[3]), stoi(argv[4]));
